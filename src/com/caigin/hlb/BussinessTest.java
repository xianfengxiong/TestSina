...
public class BussinessTest {
  ...
  public void create_activate_member() throws Exception {
    ...
    String sign = SignUtil.sign(preSignString, params.get("sign_type"), Config.getSignPrivateKey(),
        params.get(INPUT_CHARSET));
    ...
    String returnString = URLDecoder.decode(back, CHARSET);

    Map<String, String> content = GsonUtil.fromJson2Map(returnString);

    boolean success = SignUtil.checkSign(Tool.buildPreSignString(content), content.get("sign"),
        content.get("sign_type"), Config.getSignPublicKey(), content.get("_input_charset"));// Tool.buildPreSignString(content);
    if (success)
  ...
  public void set_real_name() throws Exception {
    ...
    String preSignString = Tool.buildPreSignString(params);
    String sign = SignUtil.sign(preSignString, params.get("sign_type"), Config.getSignPrivateKey(),
        params.get("_input_charset"));
    params.put("sign", sign);
    String queryString = Tool.buildQueryString(params);
    String backString = CallServiceUtil.sendPost(Config.getUrl(), queryString);
    backString = URLDecoder.decode(backString, "UTF-8");
    Map<String, String> backMap = GsonUtil.fromJson2Map(backString);
    boolean success = SignUtil.checkSign(Tool.buildPreSignString(backMap), backMap.get("sign"),
        backMap.get("sign_type"), Config.getSignPublicKey(), backMap.get("_input_charset"));
    if (success)
      System.out.println(backString);
  }
  
  public void binding_verify(){
    Map<String,String> 
  }
